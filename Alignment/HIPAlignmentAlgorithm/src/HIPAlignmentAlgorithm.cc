...
void HIPAlignmentAlgorithm::initialize(
  ...
  if (theApplyAPE){
    ...
    for (
      std::vector<edm::ParameterSet>::const_iterator setiter = theAPEParameterSet.begin();
      ...
      if (apeSPar.size() != 3  ||  apeRPar.size() != 3) throw cms::Exception("BadConfig")
...
void HIPAlignmentAlgorithm::startNewLoop(void){
  ...
  //
  AlignablePositions theAlignablePositionsFromFile = theIO.readAlignableAbsolutePositions(
    theAlignables,
...
void HIPAlignmentAlgorithm::terminate(const edm::EventSetup& iSetup){
  ...
  // write user variables	
  if (isCollector) HIPIO.writeHIPUserVariables(
    theAlignables,
    suvarfile.c_str(), theIteration, false, ioerr
    );

  // write new absolute positions to disk
  theIO.writeAlignableAbsolutePositions(
    theAlignables,
  ...
  for (ConstTrajTrackPairCollection::const_iterator it=tracks.begin(); it!=tracks.end(); ++it){
    ...
    while (itsos != tsosvec.end()){
      ...
      if (ali!=0){
        ...
        if (ihitwt!=0.){
          if (nhitDim==1) processHit1D(alidet, ali, *itsos, *ihit, ihitwt);
          else if (nhitDim==2) processHit2D(alidet, ali, *itsos, *ihit, ihitwt);
        }
      }
...
void HIPAlignmentAlgorithm::collector(void){
  ...
  for (int ijob=1; ijob<=theCollectorNJobs; ijob++){
    ...
    std::vector<AlignmentUserVariables*> uvarvec = HIPIO.readHIPUserVariables(
      theAlignables, uvfile.c_str(),
      theIteration, ioerr
      );
